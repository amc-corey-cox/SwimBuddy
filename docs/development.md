...
Commands, CI, previews and the deploy path. What the app is meant to be is in
[`spec.md`](spec.md), the order it gets built in is [`roadmap.md`](roadmap.md), and how
development is done — conventions, testing rules, the architecture boundaries — is in
[`../CLAUDE.md`](../CLAUDE.md).
...
ESLint enforces the architecture boundary rather than trusting it: `src/core/` may not
reference the DOM, IndexedDB or `localStorage`, and may not import `src/storage/` or
`src/ui/`.
...
`schema/swimbuddy.yaml` is a LinkML schema and the single source of truth for the data
model. `src/core/model.ts` and `schema/swimbuddy.schema.json` are generated from it and
committed, and are never hand-edited. Change the model by changing the schema and
running `npm run schema:gen`; see [`../schema/README.md`](../schema/README.md) for why
the schema is written the way it is.

LinkML is a Python tool and deliberately not an npm dependency. Nothing in CI, the app
build or `npm test` may need it — the committed output is what everything else reads.
`npm run schema:setup` builds the toolchain and applies `schema/patches/`, which carry a
`gen-typescript` fix that has gone upstream and is not yet released; `schema:gen` refuses
to run without them.
...
corpus, as the history the adaptation rules are tested against, and as the seed data
that makes PR previews show a populated app. See
